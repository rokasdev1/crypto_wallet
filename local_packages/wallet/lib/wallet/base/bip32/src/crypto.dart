...
import 'package:og_wallet_sdk/wallet/base/base58/base58.dart';
...
ExtendedPrivateKey deriveExtendedPrivateChildKey(
  ...
  return ExtendedPrivateKey(
    ...
    parentFingerdebugPrint: parent.fingerdebugPrint,
  );
}
...
abstract class ExtendedKey {
  ...
  /// 4 bytes
  Uint8List? parentFingerdebugPrint;
  ...
  Uint8List get fingerdebugPrint;
  ...
  List<int> _serialize() {
    ...
    serialization.addAll(parentFingerdebugPrint!);
...
class ExtendedPrivateKey extends ExtendedKey {
  ...
  factory ExtendedPrivateKey.deserialize(Uint8List key) {
    ...
    if (!extendedPrivateKey.verifyChecksum(
      sublist(
        key,
        lengthOfSerializedKey,
        lengthOfSerializedKey + lengthOfChecksum,
      ),
    )) {
  ...
  ExtendedPublicKey publicKey() {
    return ExtendedPublicKey(
      ...
      childNumber: childNumber,
      chainCode: chainCode,
      parentFingerdebugPrint: parentFingerdebugPrint,
    );
  }

  @override
  Uint8List get fingerdebugPrint => publicKey().fingerdebugPrint;
...
void debug(List<int> payload) {
  debugPrint('version: ${payload.getRange(0, 4)}');
  debugPrint('depth: ${payload.getRange(4, 5)}');
  debugPrint('parent fingerdebugPrint: ${payload.getRange(5, 9)}');
  debugPrint('childNumber: ${payload.getRange(9, 13)}');
  debugPrint('chaincode: ${payload.getRange(13, 46)}');
  debugPrint('key: ${payload.getRange(46, 78)}');
  debugPrint('checksum: ${payload.getRange(78, 82)}');
}
