...
class AbiUtil {
  ...
  static Uint8List encodeSingle(String type, dynamic arg) {
    ...
    if (type == 'address') {
      ...
      if (arg is! List) {
        throw ArgumentError('Not an array?');
      }
      size = parseTypeArray(type);
      if (size != 'dynamic' && size != 0 && arg.length > size) {
        throw ArgumentError('Elements exceed array size: $size');
      }
      var ret = BytesBuffer();
      ...
      if (size < 1 || size > 32) {
        throw ArgumentError('Invalid bytes<N> width: $size');
      }
      ...
      if ((size % 8 > 0) || (size < 8) || (size > 256)) {
        throw ArgumentError('Invalid uint<N> width: $size');
      }
      ...
      if (num < BigInt.zero) {
        throw ArgumentError('Supplied uint is negative');
      }
      ...
      size = parseTypeN(type);
      if ((size % 8 != 0) || (size < 8) || (size > 256)) {
        throw ArgumentError('Invalid int<N> width: $size');
      }
      ...
      if (num < BigInt.zero) {
        throw ArgumentError('Supplied ufixed is negative');
      }
    ...
    throw ArgumentError('Unsupported or invalid type: $type');
  }
  ...
  static Uint8List solidityPack(List<String> types, values) {
    if (types.length != values.length) {
      throw ArgumentError('Number of types are not matching the values');
    }
    var ret = BytesBuffer();
    for (var i = 0; i < types.length; i++) {
      var type = elementaryName(types[i]);
    ...
    if (isArray(type)) {
      ...
      if (!isArray(subType)) {
        ...
        if (arraySize != 'dynamic' &&
          ...
          throw ArgumentError('Elements exceed array size: ' + arraySize);
        }
      }
      var ret = BytesBuffer();
      value?.forEach((v) {
  ...
  static int parseTypeN(String type) {
    return int.parse(
      RegExp(r'^\D+(\d+)$').firstMatch(type)?.group(1) ?? '1',
      radix: 10,
    );
  }
  ...
  static BigInt parseNumber(dynamic arg) {
    if (arg is String) {
      ...
      throw ArgumentError('Argument is not a number');
    }
  }
