...
class SignatureUtil {
  ...
  static Uint8List publicKeyToAddress(Uint8List publicKey) {
    assert(publicKey.length == 64, 'public key length is invalid');
    final hashed = keccak256(publicKey);
    assert(hashed.length == 32, 'hashed length is invalid');
  ...
  static bool isValidSignature(
    ...
    var secp256k1NDiv2 = decodeBigInt(
      hex.decode(
        '7fffffffffffffffffffffffffffffff5d576e7357a4501ddfe92f46681b20a0',
      ),
    );
    var secp256k1N = decodeBigInt(
      hex.decode(
    ...
    if (homesteadOrLater && s > secp256k1NDiv2) {
      return false;
    }
